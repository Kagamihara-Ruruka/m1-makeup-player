...
m1_player/app_qt.py
  PySide6 UI。它只控制同步、字幕、進度、回寫調度與 mpv 播放核心，不直接理解 Notion MCP 或 Notion API 細節。右側上方黑色區塊是嵌入式 mpv 播放器宿主；播放控制列緊貼播放器下方，提供播放、從頭、快退快進、倍速、CC 與全螢幕；字幕區分為播放器浮動 CC、當前提詞大字區與可雙擊跳轉的字幕列表；選中影片會顯示來源解析、播放可行性、字幕與進度摘要。
```
...
主視窗標題是 `BDDE38補課系統 by RRK`。啟動時會重新同步 Notion 課程安排，列表不寫死在本機。

右側播放器採 KMP / PotPlayer 類型的操作習慣：影片本體在上方，控制列緊貼影片下方。雙擊影片區或按 `F` 可切換全螢幕，按 `Esc` 或全螢幕圖示回到一般視窗。全螢幕時會隱藏課表、影片狀態、完成回寫與字幕列表，只保留播放器、浮動 CC 字幕、時間軸與播放控制。

播放控制列包含：

- `▶` / `⏸`：未播放時顯示播放，播放中顯示暫停；若已有播放紀錄，會接續上次進度。
- `⏮`：把目前影片 seek 回 `00:00` 並立即播放。
- `⏪` / `⏩`：快退或快進 15 秒。
- 時間軸：可拖曳，也可直接點擊跳到指定位置。
- `倍速`：支援 0.5x 到 8x；交給 mpv 保持正常音高，避免變成娃娃音。
- `💬`：開啟字幕選單，可選自動、mpv 原生字幕、影片浮層字幕、下方字幕條或關閉字幕。
- `⛶`：進入播放器優先模式，管理資訊退到背景。

字幕同一時間只走一個出口。自動模式下，`.srt/.vtt/.ass` 由 mpv 原生字幕顯示；非 mpv 字幕格式在一般視窗用下方字幕條，全螢幕用影片浮層字幕。手動模式可在 `💬` 選單中指定。

字幕生成永遠讀取原始串流音訊窗格。播放器倍速只作用在 mpv 播放端，不會把 2x、6x 或 8x 之後的變速音軌送進 Whisper；字幕 worker 只接收原始 media URL 與 `start_sec/max_duration_sec` 時間窗。

目前不把字幕快取當成可信真理。選取影片時不會自動載入本地舊 sidecar；播放觸發的解析以目前時間窗為主，本輪解析完成後才把結果載入播放器。後續若要長期保存字幕，需要先補清洗與校正流程。
...
播放與字幕生成共用同一條時間軸入口。按下播放時，若目前影片沒有可用字幕、只有 `待補字幕` 佔位 cue，或既有字幕沒有覆蓋目前播放頭，播放器會自動以 `playback_timeline` 觸發背景字幕生成。時間軸觸發時會優先生成播放頭附近的短窗格，而不是從 0 秒開始硬啃整節課；已有覆蓋目前播放頭的可用字幕時則只做播放切換。這讓字幕生成依附於使用者真正要看的影片，不會在瀏覽課表或選片時提前大量消耗 Notion 串流與 GPU 資源。
