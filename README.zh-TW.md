...
2026-06-14 的實測口徑顯示，整節課一次生成字幕會讓使用者只看到最後一次交貨，和串流字幕體感衝突。rolling sidecar 應改用短窗格逐段落地。以 `medium / cuda / float16` 測同一支 Notion 影片，30 秒窗格常駐 worker 口徑約 6.08x，不足以穩追 8x；60 秒窗格在 T=480 秒約 11.94x、T=540 秒約 9.23x，可以追上 8x。模型載入約 2 秒，正式架構應讓 Whisper worker 常駐，避免每個窗格都重新載入模型。握手時間約 12 到 16 秒，屬於串流預熱，不應混入 ASR 演算法速度判斷。
