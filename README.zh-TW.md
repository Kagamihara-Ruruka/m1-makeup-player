...
右側播放器採 KMP / PotPlayer 類型的操作習慣：影片本體在上方，控制列緊貼影片下方。雙擊影片區或按 `F` 可切換全螢幕，按 `Esc` 或「離開全螢幕」回到一般視窗。全螢幕時會隱藏課表、影片狀態、完成回寫與字幕列表，只保留播放器、浮動 CC 字幕、時間軸與播放控制。

播放控制列包含：

- `播放`：播放或暫停目前影片；若已有播放紀錄，會接續上次進度。
- `從頭`：把目前影片 seek 回 `00:00` 並立即播放。
- `-15` / `+15`：快退或快進 15 秒。
- 時間軸：可拖曳，也可直接點擊跳到指定位置。
- `倍速`：支援 0.5x 到 8x；交給 mpv 保持正常音高，避免變成娃娃音。
- `CC`：切換字幕顯示。播放器會優先顯示本地 sidecar 字幕；缺字幕或字幕未覆蓋目前播放頭時，播放流程會觸發背景字幕生成。
- `全螢幕`：進入播放器優先模式，管理資訊退到背景。
